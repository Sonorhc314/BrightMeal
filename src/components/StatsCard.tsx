...
export function StatsCard({ label, value, icon, trend, accent = 'green' }: StatsCardProps) {
  return (
    <div className="rounded-2xl border border-border bg-white p-3 lg:p-4 shadow-sm transition-all hover:shadow-md">
      <div className="mb-2 flex items-center justify-between">
        <div className={`flex h-8 w-8 lg:h-9 lg:w-9 items-center justify-center rounded-lg ${accentStyles[accent]}`}>
          {icon}
        </div>
        {trend && (
          <span className="text-xs font-medium text-brand-green">{trend}</span>
        )}
      </div>
